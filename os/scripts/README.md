...
./scripts/release.sh --promote-stable   # Promote latest beta tag to a stable CI build
...
- Updates the root workspace version in `Cargo.toml` and mechanically syncs only local `rhythm-*` package versions in the root `Cargo.lock`. It does not run Cargo dependency resolution, which avoids unrelated `rhythm-chipd` lockfile churn on macOS release hosts.
...
- Binary-only rpiz releases reuse the newest rootfs image already present in the candidate OTA feeds; beta releases can borrow the stable feed's newer image. Full-image entries are replaced when `--with-image` dispatches `rpiz-sd-image.yml` and publishes `sdcard.img*` or `rootfs.ext2*`.
