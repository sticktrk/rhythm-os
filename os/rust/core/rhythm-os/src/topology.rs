...
impl RoomTopologyStore {
    ...
    /// Return the explicit automation owner for one room/hub pair. `None`
    /// means the user has not reviewed the room, which callers must treat as
    /// external-controller owned.
