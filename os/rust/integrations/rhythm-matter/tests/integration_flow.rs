...
fn dimmable_device_gets_level_only_no_color_temp() {
    let (runtime, spy) = make_pipeline_with_caps(&[
        ("matter-42", LightCapabilities::defaults_for(LightType::Dimmable)),
    ]);
    ...
    assert!(has_level, "Dimmable device should get Level Control command");
    assert!(!has_color, "Dimmable device should NOT get Color Control command");
}

#[test]
fn mixed_room_each_device_gets_appropriate_commands() {
    let (runtime, spy) = make_pipeline_with_caps(&[
        ("matter-10", LightCapabilities::defaults_for(LightType::ExtendedColor)),
        ("matter-20", LightCapabilities::defaults_for(LightType::Dimmable)),
    ]);
...
fn on_off_device_gets_on_command_only() {
    let (runtime, spy) = make_pipeline_with_caps(&[
        ("matter-42", LightCapabilities::defaults_for(LightType::OnOff)),
    ]);
...
fn unknown_device_falls_back_to_extended_color() {
    ...
    assert!(has_level, "Unknown device should get Level Control (fallback)");
    assert!(has_color, "Unknown device should get Color Control (fallback)");
}
