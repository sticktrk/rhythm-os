...
mod tests {
    ...
    #[test]
    fn moves_device_between_hue_rooms_and_verifies_membership() {
    ...
    #[test]
    fn rolls_back_source_room_when_target_room_update_fails() {
        let transport = Arc::new(SpyHueTransport::new());
        transport.set_resource_response(
            "room",
            serde_json::json!({
        ...
        let rooms = fetch_room_memberships(&transport, "user").unwrap();
        assert!(membership_matches(&rooms, "moved", Some("nook")));
        let updates = transport
            .calls()
            .into_iter()
            .filter_map(|call| match call {
