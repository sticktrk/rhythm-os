#!/bin/bash
# Build complete Docker addon image
#
# Usage: ./scripts/build-addon.sh [--arch amd64|aarch64|armv7|all] [--push]
# Orchestrates: build-rust -> build-flutter-web -> docker build
# Output: Docker image dtconcepts/rhythm-lighting-addon

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"

# Defaults
ARCH="amd64"
PUSH=false
SKIP_RUST=false
SKIP_FLUTTER=false

# Parse arguments
while [[ $# -gt 0 ]]; do
    case $1 in
        --arch)
            ARCH="$2"
            shift 2
            ;;
        --push)
            PUSH=true
            shift
            ;;
        --skip-rust)
            SKIP_RUST=true
            shift
            ;;
        --skip-flutter)
            SKIP_FLUTTER=true
            shift
            ;;
        -h|--help)
            echo "Usage: $0 [OPTIONS]"
            echo ""
            echo "Options:"
            echo "  --arch <arch>     Target architecture: amd64, aarch64, armv7, or all"
            echo "  --push            Push image to registry after build"
            echo "  --skip-rust       Skip Rust build (use existing binaries)"
            echo "  --skip-flutter    Skip Flutter build (use existing)"
            echo "  -h, --help        Show this help"
            echo ""
            echo "Output: Docker image dtconcepts/rhythm-lighting-addon"
            exit 0
            ;;
        *)
            echo "Unknown option: $1"
            exit 1
            ;;
    esac
done

# Build Rust binaries
if [ "$SKIP_RUST" = false ]; then
    echo "=== Building Rust binaries ==="
    if [ "$ARCH" = "all" ]; then
        "$SCRIPT_DIR/build-rust.sh" --release --target all
    else
        "$SCRIPT_DIR/build-rust.sh" --release --target "$ARCH"
    fi
    echo ""
fi

# Build Flutter web
if [ "$SKIP_FLUTTER" = false ]; then
    echo "=== Building Flutter web ==="
    "$SCRIPT_DIR/build-flutter-web.sh" --context ha_addon
    echo ""
fi

# Copy dist outputs to addon for Docker build
echo "=== Preparing addon directory ==="
# Addon directory (may be in a submodule)
if [ -d "$PROJECT_ROOT/rhythm-lighting/install/addon" ]; then
    ADDON_DIR="$PROJECT_ROOT/rhythm-lighting/install/addon"
else
    ADDON_DIR="$PROJECT_ROOT/install/addon"
fi

# Copy binaries
rm -rf "$ADDON_DIR/bin"
cp -r "$PROJECT_ROOT/dist/bin" "$ADDON_DIR/bin"
echo "Copied binaries to install/addon/bin/"

# Copy Flutter web
rm -rf "$ADDON_DIR/flutter_web"
cp -r "$PROJECT_ROOT/dist/flutter_web" "$ADDON_DIR/flutter_web"
echo "Copied Flutter web to install/addon/flutter_web/"

# Build Docker image
echo ""
echo "=== Building Docker image ==="
cd "$ADDON_DIR"

# Map arch to Docker platform (bash 3.2 compatible)
get_docker_platform() {
    case "$1" in
        amd64)   echo "linux/amd64" ;;
        aarch64) echo "linux/arm64" ;;
        armv7)   echo "linux/arm/v7" ;;
        *)       echo "linux/amd64" ;;
    esac
}

if [ "$ARCH" = "all" ]; then
    PLATFORMS="linux/amd64,linux/arm64,linux/arm/v7"
else
    PLATFORMS=$(get_docker_platform "$ARCH")
fi

IMAGE_NAME="dtconcepts/rhythm-lighting-addon"

if [ "$PUSH" = true ]; then
    docker buildx build \
        --platform "$PLATFORMS" \
        --push \
        -t "$IMAGE_NAME:latest" \
        .
else
    # For local builds, can only do single platform
    docker build \
        --build-arg TARGETARCH="$ARCH" \
        -t "$IMAGE_NAME:latest" \
        .
fi

echo ""
echo "Docker build complete: $IMAGE_NAME"
