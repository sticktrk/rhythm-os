...
class _FakeRhythmServerApi extends RhythmServerApi {
  ...
  final List<
    ({String hubType, String deviceId, String? hubAddress, bool force})
  >
  unpairCalls = [];
  ...
  final List<({String nodeId, int kelvin, bool preserveBrightness})>
  ...
  Future<Map<String, dynamic>?> createTopologyRoom(String name) async {
    ...
    return {'id': 'room-created', 'name': name};
  }
...
class _FakeRhythmConnection extends RhythmConnection {
  ...
  final List<({String host, int port, bool useSsl, String? authToken})>
  connectCalls = [];
...
class _HelloRhythmConnection extends _FakeRhythmConnection {
  ...
  final _hubEventController =
      StreamController<
        ({String event, String? hubType, String? address})
      >.broadcast();
  ...
  void emitHubEvent({required String event, String? hubType, String? address}) {
    _hubEventController.add((event: event, hubType: hubType, address: address));
  }
...
Future<void> _pumpRoomSettingsSheet(
  ...
  await tester.pumpWidget(
    _buildTestApp(
      ...
      child: TickerMode(
        enabled: false,
        child: RoomSettingsSheet(room: room, enableLivePreview: false),
      ),
    ),
  );
...
Future<void> _selectRoomSettingsTab(WidgetTester tester, String label) async {
...
Future<void> _loadRoomScheduleEvidenceFont() async {
  ...
  final loader = FontLoader('CodexReadableRoboto')
    ..addFont(font.readAsBytes().then((bytes) => bytes.buffer.asByteData()));
  await loader.load();
  final icons = File(
...
RhythmSceneDefinition _testScene(String id) => RhythmSceneDefinition(
  ...
  light: const RhythmLightScene(
    defaultOutput: RhythmLightSceneOutput.on(
      brightness: 55,
      color: RhythmLightColor.rgb(RhythmSceneRgbColor(r: 240, g: 80, b: 24)),
    ),
  ),
);
  ...
  light: const RhythmLightScene(
    palette: [
      RhythmLightSceneOutput.on(
        brightness: 42,
        color: RhythmLightColor.rgb(RhythmSceneRgbColor(r: 40, g: 90, b: 210)),
      ),
      RhythmLightSceneOutput.on(
        brightness: 80,
        color: RhythmLightColor.rgb(RhythmSceneRgbColor(r: 250, g: 80, b: 40)),
      ),
    ],
  ),
);
...
RhythmSceneDefinition _testHuePaletteScene(String id) =>
    _testPaletteScene(id).copyWith(
      source: RhythmSceneSource.imported(provider: 'hue', externalId: id),
...
RhythmSceneDefinition _testOffScene(String id) => RhythmSceneDefinition(
  id: id,
  name: 'Off Scene',
  light: const RhythmLightScene(defaultOutput: RhythmLightSceneOutput.off()),
);
...
RhythmSceneDefinition _testPresetOnlyScene(String id) =>
    RhythmSceneDefinition.fromJson({
      ...
      'light': {
        'entries': [
          {
            'target': {'kind': 'node', 'node_id': 'room-1'},
            'value': {'kind': 'preset', 'preset': 'reset'},
          },
        ],
      },
    });
...
void main() {
  test(
    'additional motion preserves explicit targets over physical placement',
    () {
      expect(
  ...
  test('account sign-in arms periodic activity cloud recovery', () async {
    ...
    authStates.add(const AuthUser(id: 'signed-in-owner', isAnonymous: false));
    await Future<void>.delayed(Duration.zero);
  ...
  group('ServerSyncProvider.applyProfileConfig', () {
    ...
    test('replaces an existing cached profile with absorbed widths', () {
      ...
      final cached = provider.profiles.firstWhere(
        (profile) => profile.id == 'rhythm',
      );
      expect(
  ...
  group('ServerSyncProvider.pushHubCredentials', () {
    ...
    test(
      'refuses Hue credentials when the server lacks consent support',
      () async {
        final provider = ServerSyncProvider(
          connection: connection,
          roomProvider: roomProvider,
    ...
    test('pushes Home Assistant credentials with token payload', () async {
      ...
      final hubConnected = await provider.pushHubCredentials(
  ...
  group('ServerSyncProvider room schedule authority', () {
    ...
    testWidgets('supports rooms and unassigned bulbs but not assigned bulbs', (
      ...
      connection.emitHello(
        scheduleHello(kind: 'light_device', name: 'Porch Bulb'),
      );
      await tester.pump();
      expect(provider.roomScheduleSupportedForNode('room-1'), isTrue);
    ...
    testWidgets('explains that an assigned bulb inherits room settings', (
      ...
      expect(
        find.byKey(const ValueKey('light-schedule-inherited')),
        findsOneWidget,
      );
      expect(
        find.text(
    ...
    testWidgets('installs the full authoritative write response', (
      ...
      final api = _FakeRhythmServerApi()
        ..roomScheduleSetResponse = RhythmRoomState.fromJson({
          ...
          'profile_settings': {'room_schedule': schedule('07:15').toJson()},
        });
      final connection = _HelloRhythmConnection(api);
      final provider = ServerSyncProvider(
        connection: connection,
    ...
    testWidgets('rapid edits ignore the stale first completion', (
      ...
      second.complete(
        RhythmRoomState.fromJson({
          'node_id': 'room-1',
          'state': 'active',
          'profile_settings': {'room_schedule': schedule('08:00').toJson()},
        }),
      );
    ...
    testWidgets('a thrown write rolls back and always clears pending', (
      ...
      expect(
        await provider.setRoomSchedule('room-1', schedule('07:15')),
        isFalse,
      );
  ...
  group('ServerSyncProvider room light profile overrides', () {
    ...
    testWidgets(
      'capability gates writes and supported save preserves siblings',
      (tester) async {
        final roomProvider = RoomProvider();
        final api = _FakeRhythmServerApi();
        final connection = _HelloRhythmConnection(api);
        ...
        expect(call.profileOverrides, {
          'rhythm': {'min_brightness': 8, 'max_brightness': 72},
        });
      },
    );
    ...
    testWidgets('existing override contract accepts light-device nodes', (
          ...
          'nodes': [
            {
              ...
              'time_offset': 0.0,
              'brightness_offset': 0.0,
              'light_capabilities': {'individual_profile_overrides': true},
            },
          ],
        }),
      );
      ...
      expect(provider.lightProfileOverridesSupportedForNode('light-1'), isTrue);
      expect(
        await provider.setNodeLightProfileOverride(
          'light-1',
          profileId: 'rhythm',
          ...
          'nodes': [
            {
              ...
              'time_offset': 0.0,
              'brightness_offset': 0.0,
              'light_capabilities': {'individual_profile_overrides': false},
            },
          ],
        }),
      );
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        connection.emitHello(roomLightHello(motionActivationEnabled: true));
        await tester.pump();
    ...
    testWidgets('rejected save rolls optimistic override back', (tester) async {
      ...
      expect(api.nodeProfileOverrideCalls.single.profileOverrides, {
        'rhythm': {'min_brightness': 42, 'max_brightness': 42},
      });
      expect(
        provider
            .nodeById('room-1')
  ...
  group('ServerSyncProvider Matter capabilities', () {
    ...
    test(
      'uses device_onboarding_methods from the hello capabilities payload',
      () async {
    ...
    test(
      'exposes Hue BLE only for its advertised nearby-scan method',
      () async {
        final provider = ServerSyncProvider(
          connection: connection,
          roomProvider: roomProvider,
        ...
        expect(provider.canScanToAddDeviceType(RhythmDeviceType.light), isTrue);
        expect(
    ...
    test(
      'exposes local-BLE QR only for an explicitly advertised profile',
      () async {
        final provider = ServerSyncProvider(
          connection: connection,
          roomProvider: roomProvider,
    ...
    test(
      'routes a known parser alias to the advertised current BLE profile',
      () async {
        final provider = ServerSyncProvider(
          connection: connection,
          roomProvider: roomProvider,
        ...
        expect(provider.supportedLocalBleProfileIds, {
          RhythmDeviceProfileId.oreinOc02001Button,
        });
        expect(
        ...
        expect(
          provider.canAddLocalBleProfile('orein.oc02001.button.v2'),
          isFalse,
        );
      },
    );
    ...
    test(
      'offers Hue serial intake only for an advertised connected bridge',
      () async {
        final provider = ServerSyncProvider(
          connection: connection,
          roomProvider: roomProvider,
  ...
  group('ServerSyncProvider location reconciliation', () {
    ...
    test('accepts server location when the local home has none', () async {
      ...
      final homeProvider = _TestHomeProvider(const [], currentHome: home);
      final provider = ServerSyncProvider(
        connection: connection,
        roomProvider: roomProvider,
        homeProvider: homeProvider,
  ...
  group('ServerSyncProvider topology wiring', () {
    ...
    test(
      'topology mutation refresh reports a preserved stale topology',
      () async {
        api.topologyNodes = [
          RhythmTopologyNode.fromJson({
            'id': 'room-1',
      ...
      api.topologyNodes = [
        ...
        RhythmTopologyNode.fromJson({
          ...
          'controls': [
            ...
            {'kind': 'motion', 'target_id': 'room-2', 'inherited': false},
          ],
        }),
      ];
    ...
    test(
      'control target save fails when authoritative refresh fails',
      () async {
        api.topologyNodes = [
          RhythmTopologyNode.fromJson({
            'id': 'button-1',
    ...
    test(
      'control target save survives the nodes-changed reconnect race',
      () async {
        api.topologyNodes = [
          RhythmTopologyNode.fromJson({
            'id': 'room-1',
    ...
    test('uses observed_power lights_on for room visuals', () async {
      ...
      final helloRoom = provider.helloRooms.singleWhere(
        (entry) => entry.id == 'room-1',
      );
  ...
  group('ServerSyncProvider input bindings', () {
    ...
    test(
      'does not delete stale binding when create already replaced it',
      () async {
    ...
    test(
      'treats missing binding enable toggles as successful no-ops',
      () async {
        final provider = ServerSyncProvider(
          connection: connection,
          roomProvider: roomProvider,
  ...
  group('ServerSyncProvider SSE sync', () {
          ...
          'nodes': [
            {
              ...
              'warning_active': true,
              'devices': [
                {'id': 'sensor-1', 'type': 'motion', 'name': 'Kitchen Motion'},
              ],
            },
          ],
          'location': const <String, dynamic>{},
    ...
    test('applies node_state updates to hello room summaries', () async {
      ...
      final room = provider.helloRooms.singleWhere(
        (entry) => entry.id == 'room-1',
      );
      ...
      final colorTemperature = provider.colorTemperatureCapabilitiesForNode(
        'room-1',
      );
    ...
    test(
      'applies mode_changed updates to active mode for main pills',
      () async {
        final provider = ServerSyncProvider(
          connection: connection,
          roomProvider: roomProvider,
    ...
    test(
      'runtime selection survives incremental hello without runtime fields',
      () async {
        final provider = ServerSyncProvider(
          connection: connection,
          roomProvider: roomProvider,
        ...
        connection.emitHello(
          RhythmHello.fromJson({
            ...
            'mode': {'active': 'day', 'light_runtime': 'rhythm-adaptive'},
            'location': const <String, dynamic>{},
          }),
        );
        await Future<void>.delayed(const Duration(milliseconds: 10));
    ...
    test(
      'does not infer runtime selection from day profile while asleep',
      () async {
        final provider = ServerSyncProvider(
          connection: connection,
          roomProvider: roomProvider,
    ...
    test(
      'preserves active mode across transient reconnect after hello',
      () async {
        final provider = ServerSyncProvider(
          connection: connection,
          roomProvider: roomProvider,
    ...
    test(
      'direct reconnect clears server identity until replacement hello',
      () async {
        final provider = ServerSyncProvider(
          connection: connection,
          roomProvider: roomProvider,
        ...
        connection.emitHello(
          RhythmHello.fromJson({'server_instance_id': 'srv-box-a'}),
        );
        await Future<void>.delayed(const Duration(milliseconds: 10));
        ...
        connection.emitConnectionState(RhythmConnectionState.connected);
        connection.emitHello(
          RhythmHello.fromJson({'server_instance_id': 'srv-box-b'}),
        );
        await Future<void>.delayed(const Duration(milliseconds: 10));
    ...
    test(
      'settings_changed without power_save preserves legacy cache',
      () async {
        final provider = ServerSyncProvider(
          connection: connection,
          roomProvider: roomProvider,
        ...
        connection.emitHello(
          RhythmHello.fromJson({
            'nodes': const <Map<String, dynamic>>[],
            'settings': {'power_save': false, 'auto_update': true},
            'location': const <String, dynamic>{},
          }),
        );
        await Future<void>.delayed(const Duration(milliseconds: 10));
    ...
    test(
      'sets light breaker optimistically and rolls back on failure',
      () async {
        final provider = ServerSyncProvider(
          connection: connection,
          roomProvider: roomProvider,
    ...
    test(
      'applies motion timer SSE updates to room provider and hello rooms',
      () async {
        final provider = ServerSyncProvider(
          connection: connection,
          roomProvider: roomProvider,
    ...
    test(
      'updates only the addressed hub when multiple hubs share a type',
      () async {
        final provider = ServerSyncProvider(
          connection: connection,
          roomProvider: roomProvider,
        ...
        connection.emitHello(
          RhythmHello.fromJson({
            ...
            'location': const <String, dynamic>{},
            'hubs': [
              {'type': 'hue', 'address': '192.168.1.10:443', 'connected': true},
              {'type': 'hue', 'address': '192.168.1.11:443', 'connected': true},
            ],
          }),
        );
        await Future<void>.delayed(const Duration(milliseconds: 10));
  ...
  group('ServerSyncProvider curve modifiers', () {
    ...
    test('dispatches direct brightness without editing the curve modifier', () {
      ...
      expect(dispatched, isTrue);
      expect(api.nodeBrightnessCalls, [(nodeId: 'room-1', brightness: 37)]);
  ...
  group('ServerSyncProvider mood scenes', () {
    ...
    test('keeps native scene catalogs isolated by room', () async {
      ...
      expect(provider.scenesForRoom('room-1').map((scene) => scene.id), [
        'rhythm-scene',
        'native-hue-room-1',
      ]);
      expect(provider.scenesForRoom('room-2').map((scene) => scene.id), [
        'rhythm-scene',
        'native-hue-room-2',
      ]);
      expect(
    ...
    test(
      'retains cached native scenes across a partial discovery failure',
      () async {
        api.roomScenes['room-1'] = [
        ...
        expect(fetched.map((scene) => scene.id), [
          'stored-current',
          'native-hue-aurora',
        ]);
      },
    ...
    test(
      'keeps marked and ordinary Hue scenes available for tab grouping',
      () async {
        api.roomScenes['room-1'] = [
          _testScene('rhythm-scene'),
          _testHuePaletteScene('native-hue-aurora'),
    ...
    test(
      'applies a mood scene without issuing a duplicate preferences write',
      () async {
    ...
    test(
      'stale scene failure does not roll back a newer custom mood',
      () async {
        api.scenes = [_testScene('evening-glow')];
    ...
    test('does not change optimistic scene state while disconnected', () async {
      ...
      expect(await provider.applyMoodScene('room-1', 'evening-glow'), isFalse);
      expect(provider.moodSceneIdForRoom('room-1'), isNull);
          ...
          'nodes': [
            {
              ...
              'kind': 'room',
              'state': 'mood',
              'profile_settings': {'mood_scene_id': 'node-mood-scene-room-1'},
            },
          ],
        }),
      );
          ...
          'nodes': [
            {
              ...
              'kind': 'room',
              'state': 'mood',
              'profile_settings': {'mood_scene_id': 'evening-glow'},
            },
          ],
        }),
      );
  ...
  group('ServerSyncProvider.dispatchNodeColor', () {
    ...
    test(
      'does not reuse active brightness for mood-scoped color writes',
      () async {
        await roomProvider.applyServerNodeState(
          'room-1',
          rhythmEnabled: true,
    ...
    test('does not inject brightness for non-mood color writes', () {
      final dispatched = provider.dispatchNodeColor('room-1', 255, 149, 0);
  ...
  group('ServerSyncProvider fullRefresh', () {
    ...
    test(
      'fullRefresh gates room display until the next hello arrives',
      () async {
    ...
    test(
      'scheduled hub startup keeps rooms gated until a connected hello',
      () async {
        final helloConnection = _HelloRhythmConnection(api);
        addTearDown(helloConnection.dispose);
        final provider = ServerSyncProvider(
        ...
        helloConnection.emitHello(
          RhythmHello.fromJson({
            'hubs': [
              {'type': 'matter', 'address': 'matter', 'connected': true},
            ],
          }),
        );
        await Future<void>.delayed(const Duration(milliseconds: 10));
          ...
          'hubs': [
            {
              ...
              'address': 'local',
              'connected': false,
              'startup_retry': {'status': 'scheduled', 'attempt_count': 1},
            },
          ],
        });
    ...
    test(
      'local BLE lighting profiles can retain the room readiness gate',
      () async {
        final helloConnection = _HelloRhythmConnection(api);
        addTearDown(helloConnection.dispose);
        final provider = ServerSyncProvider(
    ...
    test(
      'manual hub retry state allows rooms so recovery banner can render',
      () async {
        final helloConnection = _HelloRhythmConnection(api);
        addTearDown(helloConnection.dispose);
        final provider = ServerSyncProvider(
    ...
    test(
      'unclassified disconnected hub gates briefly then allows recovery',
      () {
        fakeAsync((async) {
          final localRoomProvider = RoomProvider();
          final localConnection = _HelloRhythmConnection(
            _FakeRhythmServerApi(),
          );
          final provider = ServerSyncProvider(
            connection: localConnection,
            roomProvider: localRoomProvider,
          ...
          localConnection.emitHello(
            RhythmHello.fromJson({
              'hubs': [
                {'type': 'matter', 'address': 'matter', 'connected': false},
              ],
            }),
          );
          async.flushMicrotasks();
    ...
    test(
      'first owner claim queues tunnel before server connection completes',
      () async {
        final home = Home.create(
          id: 'home-1',
          name: 'Kitchen',
        ...
        expect(
          homeProvider.currentHomeHubs.single.token,
          'claimed-owner-token',
        );
    ...
    test(
      'home entry refresh gates until authoritative hello arrives',
      () async {
        final helloConnection = _HelloRhythmConnection(api);
        addTearDown(helloConnection.dispose);
        final provider = ServerSyncProvider(
    ...
    test(
      'wifi fast path refreshes an already synced Home without gate',
      () async {
        final helloConnection = _HelloRhythmConnection(api);
        addTearDown(helloConnection.dispose);
    ...
    test(
      'fails over from LAN to remote endpoint when LAN reconnects',
      () async {
        final api = _FakeRhythmServerApi();
        final connection = _HelloRhythmConnection(api);
        addTearDown(connection.dispose);
    ...
    test(
      'cellular startup skips the LAN probe when remote access is saved',
      () async {
        final api = _FakeRhythmServerApi();
        final connection = _HelloRhythmConnection(api);
        addTearDown(connection.dispose);
    ...
    test(
      'retry does not use a remote endpoint without an owner token',
      () async {
        final api = _FakeRhythmServerApi();
        final connection = _HelloRhythmConnection(api);
        addTearDown(connection.dispose);
    ...
    test('retry uses the saved tunnel endpoint before cloud refresh', () async {
      ...
      expect(connection.connectCalls.single.authToken, 'owner-token');
      expect(
        homeProvider.currentHomeHubs.single.remoteEndpoint?.host,
        'old-server.rhythm.lighting',
      );
      expect(
        provider.activeConnectionEndpoint?.host,
        'old-server.rhythm.lighting',
      );
    ...
    test(
      'retry recovers a failed saved local endpoint from cloud refresh',
      () async {
        final api = _FakeRhythmServerApi();
        final connection = _HelloRhythmConnection(api);
        addTearDown(connection.dispose);
        ...
        final localHub = Hub.server(
          ...
          port: 54448,
          token: 'owner-token',
        ).copyWith(updatedAt: DateTime.utc(2026, 6, 1), pendingSync: false);
        final homeProvider = _TestHomeProvider(
          [localHub],
          currentHome: home,
          accountHomes: [
    ...
    test(
      'refreshes authoritative state when preview tick is missing',
      () async {
        final provider = ServerSyncProvider(
          connection: connection,
          roomProvider: roomProvider,
  ...
  group('ServerSyncProvider transition saves', () {
    ...
    test(
      'stores trigger-enabled edits in the server and local cache',
      () async {
        final provider = ServerSyncProvider(
          connection: connection,
          roomProvider: roomProvider,
        ...
        expect(
          api.lastSetTransitions!.every(
            (transition) => !transition.triggerEnabled,
          ),
          isTrue,
        );
        expect(
          provider.modeTransitions.every(
            (transition) => !transition.triggerEnabled,
          ),
          isTrue,
        );
  ...
  testWidgets('room page groups controls and devices into dedicated tabs', (
    ...
    expect(
      tester.getTopLeft(lightingTab).dx,
      lessThan(tester.getTopLeft(bulbsTab).dx),
    );
    expect(
      tester.getTopLeft(bulbsTab).dx,
      lessThan(tester.getTopLeft(motionTab).dx),
    );
    expect(
      tester.getTopLeft(motionTab).dx,
      lessThan(tester.getTopLeft(buttonsTab).dx),
    );
    expect(find.byKey(const ValueKey('lighting')), findsOneWidget);
    expect(find.byKey(const ValueKey('room-settings-rename')), findsOneWidget);
    expect(find.byKey(const ValueKey('room-settings-source')), findsOneWidget);
    ...
    final addBulbSemantics = tester
        .widget<Semantics>(find.byKey(const ValueKey('room-settings-add-bulb')))
        .properties;
    ...
    expect(
      find.descendant(of: motionContent, matching: find.text('MOTION SENSORS')),
      findsOneWidget,
    );
    expect(
    ...
    expect(
      tester
          .widget<Text>(
            find.byKey(const ValueKey('room-settings-light-status-room-1')),
          )
          .data,
      'Auto',
    );
    final lightingSemantics = tester.widget<Semantics>(
      find.byKey(const ValueKey('room-settings-light-settings-room-1')),
    );
    expect(lightingSemantics.properties.label, 'Lighting');
    expect(lightingSemantics.properties.value, 'Using automatic settings');
    ...
    expect(
      tester
          .getSemantics(find.byKey(const ValueKey('segmented-tab-lighting')))
          .getSemanticsData()
          .flagsCollection
          .isSelected,
      Tristate.isTrue,
    ...
    expect(find.byKey(const ValueKey('room-schedule-time-dial')), findsNothing);
    expect(
    ...
    expect(
      find.byKey(const ValueKey('room-schedule-test-sleep')),
      findsOneWidget,
    );
    ...
    expect(
      tester
          .widget<Text>(find.byKey(const ValueKey('room-settings-room-name')))
          .data,
      'Dining Room',
    );
    semantics.dispose();
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.tap(find.descendant(of: wakeRow, matching: find.text('On')));
      await tester.pump();
      expect(
        provider.roomDefaultStateForMode('room-1', RhythmMode.day),
        'active',
      );
      ...
      final sleepRow = find.byKey(
        const ValueKey('room-schedule-presets-night-room-1'),
      );
      await tester.dragUntilVisible(
        sleepRow,
        find.byKey(const ValueKey('lighting')),
    ...
    (tester) async {
      ...
      const solarData = SolarClockData(
        ...
        twilightTimes: TwilightTimesDto(
          dawn: TwilightPhaseDto(civil: 6, nautical: 5.5, astronomical: 5),
          dusk: TwilightPhaseDto(civil: 19, nautical: 19.5, astronomical: 20),
        ),
      );
      ...
      await tester.tap(find.byKey(const ValueKey('room-schedule-wake-later')));
      await tester.pumpAndSettle();
  ...
  testWidgets('room schedule renders deterministic visual evidence states', (
    ...
    await tester.pumpWidget(
      _buildTestApp(
        ...
        child: RepaintBoundary(
          key: boundaryKey,
          child: const RoomSettingsSheet(room: room, enableLivePreview: false),
        ),
      ),
    );
    await tester.pump(const Duration(milliseconds: 10));
  ...
  testWidgets('room tabs show Scan above the inline existing-device list', (
    ...
    await tester.pump(const Duration(milliseconds: 10));
    expect(provider.canScanToAddDevice, isTrue);
    expect(provider.canScanToAddDeviceType(RhythmDeviceType.light), isFalse);
    ...
    expect(find.byKey(const ValueKey('room-device-add-sheet')), findsOneWidget);
    expect(
    ...
    expect(find.text('Wall Button'), findsNothing);
    if (const bool.fromEnvironment('RHYTHM_CAPTURE_ROOM_DEVICE_ADD_EVIDENCE')) {
      await expectLater(
        find.byKey(const ValueKey('room-device-add-sheet')),
        matchesGoldenFile('goldens/room-device-add-unassigned-first.png'),
      );
    }
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await _selectRoomSettingsTab(tester, 'Buttons');
      await tester.tap(find.byKey(const ValueKey('room-settings-add-button')));
      await tester.pumpAndSettle();
  ...
  testWidgets('room sheet adds existing motion as an additional room control', (
    ...
    await _selectRoomSettingsTab(tester, 'Motion');
    await tester.tap(find.byKey(const ValueKey('room-settings-add-motion')));
    await tester.pumpAndSettle();
    ...
    (tester) async {
      ...
      connection.emitHello(
        RhythmHello.fromJson({
          ...
          'location': const <String, dynamic>{},
          'hubs': [
            {'type': 'hue', 'connected': true},
            {'type': 'homeassistant', 'connected': true},
          ],
          'capabilities': {
            'hubs': [
              {
  ...
  group('ServerSyncProvider demo mode', () {
    ...
    test(
      'populates topology and triage data from the shared demo state',
      () async {
        final provider = ServerSyncProvider(
          connection: connection,
          roomProvider: roomProvider,
        ...
        expect(
          provider.helloRooms.map((room) => room.name),
          contains('Living Room'),
        );
        expect(provider.devicesForRoom('hue_demo_1').length, 4);
        expect(
          provider.deviceSummaryForRoom('hue_demo_1'),
    ...
    test(
      'refreshes demo topology after assigning an unassigned device',
      () async {
        final provider = ServerSyncProvider(
          connection: connection,
          roomProvider: roomProvider,
        ...
        final success = await provider.api.assignDeviceParent(
          'demo_floor_lamp',
          'hue_demo_2',
        );
        await Future<void>.delayed(const Duration(milliseconds: 20));
        ...
        RhythmHello.fromJson({
          ...
          'nodes': [
            ...
            },
            {
              ...
              'profile_settings': {
                'profile_overrides': {
                  'rhythm': {'min_brightness': 12, 'max_color_temp': 4800},
                },
              },
            },
          ],
  ...
  testWidgets('Hue Bluetooth lights expose their removable connection', (
    ...
    api.canonicalDevices['light-1'] = {
      ...
      'endpoints': [
        {
          'hub_key': {'hub_type': 'hue_ble', 'address': 'local'},
    ...
    expect(find.textContaining('powered on and nearby'), findsOneWidget);
    expect(
  ...
  testWidgets('Hue Bluetooth release failure offers retry before local forget', (
    ...
    expect(find.textContaining('may require a factory reset'), findsOneWidget);
    expect(api.unpairCalls, [
      (
        hubType: 'hue_ble',
    ...
    expect(api.unpairCalls.map((call) => call.force), [false, false]);

    await tester.tap(find.text('Forget Anyway'));
    await tester.pumpAndSettle();

    expect(api.unpairCalls.map((call) => call.force), [false, false, true]);
    expect(connection.reconnectCalls, 1);
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(find.text('Remove Device'), findsOneWidget);
      expect(
        find.text('Removed Hue Bluetooth connection from Merged Hue lamp'),
        findsOneWidget,
      );
    },
  );
  ...
  testWidgets(
    'merged Hue Bluetooth lifecycle warnings require acknowledgement',
    (tester) async {
      _registerWidgetCleanup(tester);
      final roomProvider = RoomProvider();
      ...
      expect(find.text('Removed Hue Bluetooth connection'), findsOneWidget);
  ...
  testWidgets('Hue Bridge switch removal uses typed capability and copy', (
    ...
    await tester.pumpAndSettle();
    expect(
      find.textContaining('whether the switch is already absent'),
      findsOneWidget,
    );
    ...
    expect(api.unpairCorrelationIds.single, startsWith('hue-bridge-remove-'));
  });
  ...
  testWidgets('Hue Bridge fallback only finishes after absence confirmation', (
    ...
    expect(
      find.textContaining('only after the bridge confirms the bulb is absent'),
      findsOneWidget,
    );
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.tap(
        find.byKey(const ValueKey('device-settings-light-settings-light-1')),
      );
      await tester.pumpAndSettle();
  ...
  testWidgets('group-routed Hue bulb disables individual Lighting settings', (
    ...
    expect(
      tester
          .widget<Text>(
            find.byKey(
              const ValueKey('device-settings-light-status-hue-light-1'),
            ),
          )
          .data,
      'Room only',
  ...
  testWidgets('motion controls lightly select and save multiple rooms', (
    ...
    api.topologyNodes = [
      ...
      RhythmTopologyNode.fromJson({
        ...
        'parent_id': 'room-1',
        'controls': [
          {'kind': 'motion', 'target_id': 'room-1', 'inherited': true},
        ],
      }),
    ];
    ...
    expect(find.text('Updated Kitchen Motion motion controls'), findsOneWidget);
  });
    ...
    (tester) async {
      ...
      api.topologyNodes = [
        ...
        RhythmTopologyNode.fromJson({
          ...
          'parent_id': 'room-1',
          'controls': [
            {'kind': 'button', 'target_id': 'room-1', 'inherited': true},
          ],
        }),
      ];
  ...
  testWidgets('room motion tab includes a sensor parented to another room', (
      ...
      RhythmHello.fromJson({
        'nodes': [
          ...
          },
          {
            ...
            'lights_on': true,
            'devices': [
              {'id': 'sensor-1', 'type': 'motion', 'name': 'Drop zone hallway'},
            ],
          },
        ],
        'location': const <String, dynamic>{},
    ...
    expect(find.text('MOTION SENSORS', skipOffstage: false), findsOneWidget);
    expect(find.text('Drop zone hallway', skipOffstage: false), findsOneWidget);
  ...
  testWidgets(
    'Room card device flow offers Remove from Room for Matter bulbs',
    (tester) async {
      _registerWidgetCleanup(tester);
      final roomProvider = RoomProvider();
      final api = _FakeRhythmServerApi();
  ...
  testWidgets('room settings standby switch pushes node preference', (
    ...
    await tester.tap(
      find.descendant(
        of: find.byKey(const ValueKey('room-settings-low-glow-room-1')),
  ...
  testWidgets(
    'room standby preference ignores stale hello after local change',
    (tester) async {
      final roomProvider = RoomProvider();
      final api = _FakeRhythmServerApi();
      final connection = _HelloRhythmConnection(api);
      ...
      RhythmHello helloWithStandby(bool standbyEnabled) =>
          RhythmHello.fromJson({
            'nodes': [
              {
                'id': 'room-1',
  ...
  testWidgets(
    'room settings hides motion timeout rows without motion behavior',
    (tester) async {
      _registerWidgetCleanup(tester);
      final roomProvider = RoomProvider();
      final api = _FakeRhythmServerApi();
  ...
  testWidgets('room settings Motion tab pushes day and sleep motion timeouts', (
    ...
    expect(dayCall.profileOverrides, {
      'rhythm': {
    ...
    expect(sleepCall.profileOverrides, {'sleep': null});
    expect(
      provider
          .nodeById('room-1')
          ?.profileSettings
  ...
  testWidgets(
    'Delete Room is shown for matter rooms and calls the SDK method',
    (tester) async {
      _registerWidgetCleanup(tester);
      final roomProvider = RoomProvider();
      final api = _FakeRhythmServerApi();
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.tap(find.byKey(const ValueKey('day-low-glow-custom-color')));
      await tester.pumpAndSettle();
      await tester.tap(find.text('Save'));
      await tester.pumpAndSettle();
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      tester
          .widget<Slider>(find.byKey(const ValueKey('day-low-glow-brightness')))
          .onChanged!(80);
      await tester.pumpAndSettle();
      await tester.tap(find.byKey(const ValueKey('day-low-glow-custom-color')));
      await tester.pumpAndSettle();
    ...
    Map<String, dynamic> roomHello({required bool explicitDayIdle}) => {
      ...
      'nodes': [
        ...
        },
        {
          ...
          'profile_settings': {
            'profile_overrides': {
              'day_idle': {'min_brightness': 7, 'max_brightness': 7},
            },
          },
        },
      ],
