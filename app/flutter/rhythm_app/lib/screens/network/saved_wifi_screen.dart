...
class SavedWifiScreen extends StatefulWidget {
  ...
  static Future<String?> select(
          BuildContext context, CloudBackedServerApi api) =>
...
class _SavedWifiScreenState extends State<SavedWifiScreen> {
  ...
  Future<void> _write(String action,
      {String? id, String? ssid, String? password}) async {
  ...
  Future<void> _edit([RhythmWifiProfile? profile]) async {
    final result = await showDialog<({String ssid, String? password})>(
        context: context, builder: (_) => _WifiEditor(profile: profile));
    if (result != null && mounted) {
      await _write('save',
          id: profile?.id, ssid: result.ssid, password: result.password);
    }
  }
  ...
  Widget build(BuildContext context) => Scaffold(
      appBar: AppBar(
          title: Text(widget.choose
              ? 'Choose provisioning network'
              : 'Saved Wi-Fi networks')),
      body: ListView(padding: const EdgeInsets.all(20), children: [
        const Text(
            'These networks are used to set up accessories. Changing the default does not move your Rhythm Box.'),
        const SizedBox(height: 16),
        if (_busy) const LinearProgressIndicator(),
        if (_error != null) ...[
          Text(_error!, key: const ValueKey('wifi-error')),
          TextButton(
              onPressed: _busy ? null : _load, child: const Text('Reload'))
        ],
        ...
        for (final profile in _catalog?.profiles ?? <RhythmWifiProfile>[])
          ListTile(
              ...
              leading: widget.choose
                  ? Icon(_selection == profile.id
                      ? Icons.radio_button_checked
                      : Icons.radio_button_off)
                  : const Icon(Icons.wifi),
              title: Text(profile.ssid),
              subtitle: _catalog!.defaultId == profile.id
                  ? const Text('Provisioning default')
                  : null,
              onTap: _busy
                  ? null
                  : () => widget.choose
