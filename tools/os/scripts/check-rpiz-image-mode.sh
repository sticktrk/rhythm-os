...
INITTAB_FILE="$TARGET_DIR/etc/inittab"
HOST_RECORDER_BIN="$TARGET_DIR/usr/bin/rhythm-host-recorder"
HOST_RECORDER_BOOT="$TARGET_DIR/etc/init.d/S42hostrecorder"
...
[ -x "$HOST_RECORDER_BIN" ] || fail "missing executable $HOST_RECORDER_BIN"
[ -x "$HOST_RECORDER_BOOT" ] || fail "missing executable $HOST_RECORDER_BOOT"
[ -f "$INITTAB_FILE" ] || fail "missing $INITTAB_FILE"
grep -Fq '::respawn:/usr/bin/rhythm-host-recorder run --data-dir /data' "$INITTAB_FILE" \
    || fail "host recorder is not independently supervised"
recorder_line="$(grep -nF '::respawn:/usr/bin/rhythm-host-recorder run --data-dir /data' "$INITTAB_FILE" | cut -d: -f1)"
watchdog_line="$(grep -nF '::respawn:/usr/bin/rhythm-hardware-watchdog' "$INITTAB_FILE" | cut -d: -f1)"
[ "$recorder_line" -lt "$watchdog_line" ] || fail "host recorder respawn entry must precede watchdog"
