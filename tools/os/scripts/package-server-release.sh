...
#
# Usage:
#   tools/os/scripts/package-server-release.sh --target macos-arm64 --version v0.4.176-beta --output dist/release-assets
#
...
[ -f "$SERVER_BIN" ] || { echo "Error: $SERVER_BIN not found. Build with tools/os/scripts/build-server.sh --release --target $TARGET" >&2; exit 1; }
